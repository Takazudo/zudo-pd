...
def measure_mesh(mesh,profile):
    results=[]
    for z in [1,4.1,7.9,8.1,9,10,12,13.4,13.499,13.501,13.6,14,16,17.5,17.999]:
        loops=plane_loops(mesh,z);r={'z_mm':z,'loop_count':len(loops)}
        if z>4:
            radii=np.linalg.norm(loops[0],axis=1)
            require(np.max(np.abs(radii-3.5))<.002,'Outer pole must be straightØ7mm; no top taper orØ8 section')
            r['outer_radius_mm']=[float(radii.min()),float(radii.max())]
        if z<13.5:
            require(len(loops)==1,'Unexpected cavity below the4.5mm gripping bore')
            r['solid_below_grip']=True
        else:
            require(len(loops)==2,'Expected exactly one gripping bore')
            q=loops[1];simple=simplified(q);forward=deviation(q,profile);back=deviation(profile,q);r.update(inner_vertices_after_collinear_reduction=len(simple),inner_bounds_mm=[q.min(0).tolist(),q.max(0).tolist()],area_mm2=area(q),profile_to_section_max_sampled_mm=back,section_to_profile_max_sampled_mm=forward);require(max(forward,back)<2e-6,'Gripping profile changed')
        results.append(r)
    return results
...
def mesh_checks(stl,four,leg,spec,reference,source_path):
    ...
    require(spec['screw_bore']['total_blind_depth_mm']==4.5
            and spec['screw_bore'].get('lower_clearance_diameter_mm') in (None,0)
            and spec['screw_bore']['entrance_chamfer_mm']==0
            and spec['screw_bore']['modeled_threads'] is False,
            'Only the original4.5mm blind gripping profile is permitted; no lower pocket/chamfer/threads')
    require(spec['pole']['diameter_mm']==7 and spec['pole'].get('seating_diameter_mm',7)==7
            and spec['pole'].get('top_taper_height_mm',0)==0,'StraightØ7mm pole specification required')
    ...
    for tag,m in [('STL',mesh),('STEP tessellation',tessellate(leg))]:
        ...
        for z in (7,10,13.4,13.499):
            require(len(plane_loops(m,z))==1,'Material below the gripping bore must be solid')
    # A continuous axial witness covers the formerØ3.4mm pocket at every height,
    # complementing the discrete STEP/STL sections without rebuilding the leg.
    core=cq.Solid.makeCylinder(1.8,13.48,cq.Vector(0,0,.01))
    absent_volume=core.Volume()-core.intersect(leg).Volume()
    require(abs(absent_volume)<1e-5,'A cavity remains below the4.5mm gripping bore')
    four_checks=[]
    ...
    return {'status':'PASS','original_reference':source_review,'main_stl_watertight':True,'four_up_watertight_solids':4,
            ...
            'profile_tolerance_mm':2e-6,'straight_pole_diameter_mm':7,'blind_bore_depth_mm':4.5,
            'continuous_lower_solid_core_check':{'diameter_mm':3.6,'z_interval_mm':[.01,13.49],'absent_volume_mm3':absent_volume},
            'grip_assumption':'The user-proven untapped aperture is retained exactly for4.5mm. The bore now ends at a flat blind floor; no lower enlarged cavity remains.'}


def screw_depth_checks(spec):
    thickness=spec['assembly']['board_thickness_mm'];depth=spec['screw_bore']['total_blind_depth_mm']
    require(thickness==1.6 and depth==4.5,'Unexpected board thickness or gripping depth')
    rows=[]
    for length,washer,label in [(5,0,'M3x5 without washer'),(6,.5,'M3x6 with minimum0.5mm washer'),(8,0,'Previous M3x8 without washer')]:
        insertion=round(length-thickness-washer,6);clearance=round(depth-insertion,6)
        rows.append({'configuration':label,'length_under_head_mm':length,'washer_thickness_mm':washer,
                     'insertion_mm':insertion,'gripping_engagement_mm':min(insertion,depth),
                     'blind_floor_clearance_mm':clearance,'nominal_depth_fit':'PASS' if clearance>0 else 'DOES NOT FIT'})
    for field,value in [('initial_screw_insertion_mm',3.4),('gripping_engagement_mm',3.4),('tip_clearance_to_blind_floor_mm',1.1)]:
        require(math.isclose(spec['assembly'][field],value,abs_tol=1e-9),'Stale screw-depth spec: '+field)
    require(spec['assembly']['initial_screw'].replace(' ','').lower().startswith('m3x5'),'Initial screw must be M3x5')
    return {'initial_screw':'M3x5','board_thickness_mm':thickness,'blind_depth_mm':depth,'configurations':rows,
            'optional_washer_note':'For M3x6, a washer at least0.5mm thick gives insertion at most3.9mm and floor clearance at least0.6mm.',
            'scope':'Nominal under-head screw length and stack arithmetic; separate from the user-proven gripping profile and PCB free-pass check.'}
...
def main():
    ...
    screw_report=screw_depth_checks(spec)
    ...
    report={'schema_version':1,'status':'PASS','units':'mm','source_inputs_sha256':before,'source_pcb_sha256':assembly_report['pcb_sha256'],
            ...
            'pcb_screw_passage':hole_checks,'screw_engagement':screw_report,
            'limits':['The upper untapped grip copies the user-proven source geometry. No modeled threads or replacement circular pilot were introduced.',
                      'The printed bore is4.5mm deep with no lower enlarged pocket. M3x5 is the initial nominal choice; the previous M3x8 exceeds this depth.',
