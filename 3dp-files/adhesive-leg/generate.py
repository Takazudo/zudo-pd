...
def build(spec,profile):
 ...
 assert pole['diameter_mm']==pole['seating_diameter_mm'] and pole['top_taper_height_mm']==0
 stem=cq.Workplane('XY').workplane(offset=t).circle(pole['diameter_mm']/2).extrude(height-t).val()
 body=foot.fuse(stem).clean()
 ...
 assert bore['total_blind_depth_mm']==bore['gripping_depth_mm']==4.5
 leg=body.cut(grip).clean()
...
def main():
 ...
 render([('leg',leg,(.15,.43,.51))],OUT/'preview.png','18 mm straight adhesive PCB leg | M3 rail-profile grip')
 cutter=cq.Workplane('XY').box(40,20,40).translate((0,-10,18)).val()
 cutaway=leg.cut(cutter)
 render([('section',cutaway,(.15,.43,.51))],OUT/'section.png','One 4.5 mm blind bore | solid below',elev=15,azim=-90)
