...
![LM2596S-ADJ Package Preview](/footprint-imgs/LM2596S-ADJ.png)
...
| 5   | ON/OFF   | Enable Control (Low or floating = ON; connection depends on topology - see [ON/OFF Pin Configuration](#design-considerations-8-on-off-pin-configuration)) |
...
Signal Path: +15V → [U2] → L1 → +13.5V → C3 → [LM7812] → +12V OUT
...
- ON/OFF pin connection differs between regular buck (U2, U3) and inverting buck-boost (U4) - see [ON/OFF Pin Configuration](#design-considerations-8-on-off-pin-configuration) for details
