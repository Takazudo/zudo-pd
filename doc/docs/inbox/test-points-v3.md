...
> **Note on CC1DB / CC2DB pads on a built v3 board**: because v3 hardwires both CC1DB and CC2DB to GND, the J3 pads for these signals will read **0 V** on every healthy board. They are therefore *GND-tie verifiers* (does the trace from the chip pin to GND still measure 0 Ω?), not signal-state probes. If a future chip batch turns out to have a working internal Rd and you cut the GND trace to switch back to dead-battery wiring, these same pads become live signal-state probes — no board respin needed for the pads themselves.
