...
**Power Lines:**
- `USB-C VBUS (B9, A9)` → `CH224D pin 2 (VBUS)` → `+15V Output`
- `VBUS` → `R2/Rset (56kΩ)` → `DRV/CFG1 junction`

**Communication Lines:**
- `USB-C CC1 (A5)` → `CH224D pin 10 (CC1)`
- `USB-C CC2 (B5)` → `CH224D pin 11 (CC2)`

**Ground:**
- `USB-C GND (B12, A12)` → `CH224D pin 0 (EPAD)` (thermal pad)
- `C1 negative` → `GND`
- `C2 negative` → `GND`
