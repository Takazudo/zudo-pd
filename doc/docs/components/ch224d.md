...
| 8 | PG | Power Good output (open drain, active low) | LED indicator via resistor to +5V |
| 9 | DM | USB D- data line (UDM) | **Shorted to pin 10 (DP)** for PD-only mode |
| 10 | DP | USB D+ data line (UDP) | **Shorted to pin 9 (DM)** for PD-only mode |
...
### Power Good (PG) Pin Operation

- **Open-drain output**: Requires external pull-up (via LED and resistor to +5V)
- **Active LOW**: Pulls to GND when power is good
- **LED Circuit**: When PG is LOW, current flows: +5V → R1 (330Ω) → LED1 → PG → GND
- **LED Current**: I = (5V - 2.2V) / 330Ω ≈ 8.5mA ✅
...
2. **VDD Decoupling**: Place C3 ceramic capacitor very close to pin 7 (VDD)
...
| PG LED always off | PG pin not pulled up | Verify LED circuit to +5V |
