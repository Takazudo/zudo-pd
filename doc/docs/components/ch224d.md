...
└──────────────┘ │       │              │
                 │       │    EP, GND   ├11,15── GND
                 │       │              │
               C1│10µF   │    PG ───────┤8──┬──→ GND (via LED1)
                 │       │              │   │
               C2│10µF   └──────────────┘   │
                 │                          │
                GND             +5V ──[R1: 330Ω]──[LED1: Green]──┘
                                (from Diagram7 +5V output)
...
- U1 (CH224D) PG (pin 8) → LED1 (Green, cathode) → LED1 (anode) → R1 (330Ω) → +5V rail (from Diagram7)
```
...
|-----------|------|-------|---------|-------------|---------|
| LED1 | Green LED | 2.2V @ 10mA | 0805 | C2286 | Power negotiation status indicator |
...
```
Normal Operation:
    +5V ────┬──── R1 (330Ω) ──┬──── LED1 ──┬──→ PG pin
            │                 │            │
         (from D7)        (Anode)     (Cathode)  (Open drain)

When PD is NOT negotiated (PG = High-Z):
- Internal pullup in CH224D keeps pin high (floating)
- No current flows through LED
- LED1 is OFF
- Indicates: Waiting for negotiation or fault

When PD is negotiated successfully (PG = Low):
- CH224D pulls PG pin to GND
- Current flows: +5V → R1 → LED1 → PG (GND)
- LED1 illuminates (Green)
- Indicates: 15V available and stable
```

**LED Current Calculation**:
```
I_LED = (V_5V - V_LED - V_PG) / R1
I_LED = (5V - 2.2V - 0.3V) / 330Ω
I_LED ≈ 7.6mA

✅ Safe LED current (within 20mA max specification)
✅ Good brightness for status indication
✅ Matches other status LEDs in the circuit
```

**Note**: The 330Ω resistor value is correct for the +5V rail. Previous documentation incorrectly suggested 1kΩ based on using 15V source.
...
| LED1 | LED | Green 0805 | 0805 | C2286 | 1 | $0.0115 | $0.012 |
| R1 | Resistor | 330Ω ±1% | 0603 | C23138 | 1 | $0.0005 | $0.0005 |
