...
STUSB4500 (QFN-24) - USB-IF Certified PD Sink Controller:
    ...
    RESET   6 │── R14 (10kΩ) ── VDD               19│ ALERT (NC)
...
│ RESET (6)    │ R14 (10kΩ) to VDD (active-low, default running)      │
...
- `VBUS_EN_SNK (pin 16)` → `R12 (22kΩ)` → `Q1 Gate`
...
- `RESET (pin 6)` → `R14 (10kΩ)` → `VDD` (active-low, default = running)
- `ADDR0 (pin 12)` → `GND`
- `ADDR1 (pin 13)` → `GND`
