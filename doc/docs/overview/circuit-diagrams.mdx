...
```
    ...
    │   │       GND          │                                          │
    │   │                    │                                          │
    │   │  VSYS (pin 11) ────┼── VREG_2V7 (tie together)                │
    │   │                    │                                          │
    │   │  RESET (pin 15) ───┼── R14 (10kΩ) ── VDD                      │
    │   │                    │                                          │
    │   │  DISCH (pin 21) ───┼── R13 (470Ω) ── VBUS_OUT                 │
    │   │                    │                                          │
    │   │  ADDR0 (pin 16) ───┼── GND                                    │
    │   │  ADDR1 (pin 17) ───┼── GND                                    │
    │   │                    │                                          │
...
┌─────────────────────────────────────┐
│  2, 5  VBUS  ───────────────────────┼──→ VBUS_IN Rail (5V initially)
│                                     │
...
Power Path with P-Channel MOSFET Load Switch:
                    ...
                    VBUS_IN (15V after PD negotiation)
                      ...
                      R11 (100kΩ)  ← Gate pull-up (default OFF)
                           │
VBUS_EN_SNK ───┬── R12 (22kΩ) ──┴─── Gate ─── Q1 (AO3401A)
(from STUSB4500)│                              │    │
               │                            Source Drain
              C35 (100nF)                      │    │
               │                           VBUS_IN  │
...
- Time constant: τ = R12 × C35 = 22kΩ × 100nF = 2.2ms
...
STUSB4500 (QFN-24) - USB-IF Certified PD Sink Controller:
              ...
              │              (Top View)               │
              │                                       │
     CC1DB  1 │●                                   24│ VREG_1V2 ── C34 (1µF) ── GND
       CC1  2 │                                   23│ CC2DB
       NC   3 │                                   22│ CC2
       CC2  4 │                                   21│ DISCH ── R13 (470Ω) ── VBUS_OUT
      VDD   5 │─── VBUS_IN + C2 (100nF)           20│ ATTACH (NC)
      GND   6 │─── GND                            19│ GPIO (NC)
  VREG_2V7  7 │── C30 (1µF) ── GND                18│ ALERT (NC)
      SCL   8 │── NC (or I2C)                     17│ ADDR1 ── GND
      SDA   9 │── NC (or I2C)                     16│ ADDR0 ── GND
VBUS_EN_SNK10 │──→ Gate drive circuit             15│ RESET ── R14 (10kΩ) ── VDD
     VSYS  11 │── VREG_2V7                        14│ POWER_OK3 (NC)
      GND  12 │── GND                             13│ POWER_OK2 (NC)
              │                                       │
              │         ┌──────────────┐              │
              └─────────┤  Exposed Pad ├──────────────┘
                        │     GND      │
...
│ Pin          │ Connection                                           │
├──────────────┼──────────────────────────────────────────────────────┤
│ VDD (5)      │ VBUS_IN + C2 (100nF) to GND                          │
│ CC1 (2)      │ USB-C CC1, also to CC1DB (pin 23)                    │
│ CC2 (4)      │ USB-C CC2, also to CC2DB (pin 22)                    │
│ CC1DB (23)   │ Tie to CC1 (enables dead battery mode)               │
│ CC2DB (22)   │ Tie to CC2 (enables dead battery mode)               │
│ VBUS_EN_SNK  │ To gate drive (R12) → Q1 gate                        │
│ VREG_2V7 (7) │ C30 (1µF) to GND, also to VSYS (pin 11)              │
│ VREG_1V2 (24)│ C34 (1µF) to GND                                     │
│ VSYS (11)    │ Tie to VREG_2V7                                      │
│ RESET (15)   │ R14 (10kΩ) to VDD (active-low, default running)      │
│ DISCH (21)   │ R13 (470Ω) to VBUS_OUT (for VBUS discharge)          │
│ ADDR0/1      │ Both to GND (I2C address = 0x28)                     │
│ GND (6,12)   │ System GND                                           │
│ EP (EPAD)    │ System GND (thermal pad)                             │
│ NC pins      │ 3,13,14,18,19,20 - Not connected                     │
└──────────────┴──────────────────────────────────────────────────────┘
```
...
- `USB-C VBUS (pins 2, 5)` → `VBUS_IN Rail` → `STUSB4500 VDD (pin 5)`
- `C1 (10µF ceramic)`: `VBUS_IN` ⟷ `GND` (bulk filtering)
- `C2 (100nF ceramic)`: `VDD (pin 5)` ⟷ `GND` (close to IC)

**Internal Regulators:**

- `VREG_2V7 (pin 7)` → `C30 (1µF)` → `GND`
- `VREG_1V2 (pin 24)` → `C34 (1µF)` → `GND`
- `VSYS (pin 11)` → `VREG_2V7 (pin 7)` (tie together)

**CC Lines:**

- `USB-C CC1 (pin 3)` → `STUSB4500 CC1 (pin 2)` → `CC1DB (pin 23)` (tie CC1 and CC1DB together)
- `USB-C CC2 (pin 4)` → `STUSB4500 CC2 (pin 4)` → `CC2DB (pin 22)` (tie CC2 and CC2DB together)
- Optional: `TVS4 (ESDA25L)` K1 → CC1, K2 → CC2, A → GND

**Load Switch (Power Path Control):**

- `VBUS_IN` → `R11 (100kΩ)` → `Q1 Gate` (pull-up, default OFF)
- `VBUS_EN_SNK (pin 10)` → `R12 (22kΩ)` → `Q1 Gate`
...
- `RESET (pin 15)` → `R14 (10kΩ)` → `VDD` (active-low, default = running)
- `ADDR0 (pin 16)` → `GND`
- `ADDR1 (pin 17)` → `GND`
...
- `USB-C GND (pins 1, 6)` → `System GND`
- `STUSB4500 GND (pins 6, 12)` → `System GND`
- `STUSB4500 EP (EPAD)` → `System GND` (thermal + ground)
