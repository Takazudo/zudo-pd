...
The guarded 110 × 85 mm prototype export is generated and locally verified in
`guarded-review`. Its reports bind the checks to the exported revision. Physical
fit, retention and full-load qualification remain open. The earlier
`compact-review` and `renewal-review` packages describe older geometry and must
not be used to fabricate the guarded revision.

## Generate files from checked sources

```sh
python3 scripts/pcb/export-jlcpcb.py manufacturing/releases/guarded-review
uv run --with gerbonara --with resvg-py python scripts/pcb/verify-jlcpcb.py manufacturing/releases/guarded-review
```
...
- Faston terminals are on the underside, face inward along +X, and keep their
  entire metal envelope inside the PCB. Three rounded T-notches retain a four-bay
  [printed guard, caps and J8 sliding shutter](../3dp-files/faston-cover/README.md).
  The print set contains five assembly pieces and ten fit coupons. Use the exact
  line/arc contract and the stepped J8 cable envelope in the guard instructions;
  cap/shutter retention, print fit and actual mating access remain unmeasured.
...
transforms have been corrected to their footprints. The Faston and synth-socket STEP datums are also
normalized to their WRL and pad datums by rigid translations; their solid geometry
is unchanged. These model checks do not establish physical mating fit.

The [retained STEP datum audit](../footprints/kicad/step-datum-verification.json)
checks 31 active STEP/WRL pairs after alignment. It compares sampled surfaces in
...
with the guard installed; passing geometry checks does not qualify cooling.
