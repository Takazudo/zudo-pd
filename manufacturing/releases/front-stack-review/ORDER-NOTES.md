...
The 110 × 85 mm front-stack prototype package is locally verified in
`front-stack-review`; its reports bind the checks to the recorded source revision.
Board B is installed on legs with its front/component face down. Physical wire fit,
tightening torque, leg clearance, connector seating and full-load qualification
remain open. Earlier release packages are immutable records of older assembly geometry.

## Generate files from checked sources

```sh
python3 scripts/pcb/export-jlcpcb.py manufacturing/releases/front-stack-review
uv run --with gerbonara --with resvg-py python scripts/pcb/verify-jlcpcb.py manufacturing/releases/front-stack-review
```
...
- Board B's J5 female header is on F.Cu. The PD module faces it from above B in
  ...
  Map P's XY coordinates with `pd_to_board_b(x,y)=(y,x)` for all holes and mating
  pads. The nominal plane spacing does not establish actual seated header height.
