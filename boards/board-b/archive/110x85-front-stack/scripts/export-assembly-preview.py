...
B, with P(x,y) mapping to B(y,x). All fitted parts are screened across the boards;
...
def place_board_p(shape,contract):
    ...
    require(stack['point_transform']=='P(x,y) -> B(y,x)' and stack['cad_rotation_axis']==[1,-1,0]
            and stack['cad_rotation_deg']==180 and stack['nominal_board_p_top_z_mm']==11.1,
            'Unreviewed face-to-face stack transform')
    return shape.rotate((0,0,0),(1,-1,0),180).translate((0,0,11.1))
...
def self_test():
    ...
    for x,y,z in [(6.55,-37.7,0),(19.25,-37.7,0),(4,-4,0),(23,-4,0),(12.8,-32.6,0),(0,0,1)]:
        ...
        require(math.dist((point.x,point.y,point.z),(-y,-x,11.1-z))<1e-8,'Face-to-face point transform failed')
    a=cube(37.7,-6.55,6)
    ...
    cases=[('non-mating component collision',{'C5':a},{'Q1':a}),
           ('connector-body collision outside contact columns',{'J5':cube(35,-6.55,6)},{'JOUT1':cube(35,-6.55,6)}),
           ('connector collision beyond mating depth',{'J5':cube(37.7,-6.55,10)},{'JOUT1':cube(37.7,-6.55,10)}),
    ...
    return {'status':'PASS','positive_contact_and_transform_checks':2,'negative_controls':[v[0] for v in cases]+['wrong CAD rotation']}
...
def main():
    ...
    verification_sources={str(path.relative_to(ROOT)):sha(path) for path in
                          [ROOT/'scripts/pcb/verify-board-b.py',ROOT/'scripts/pcb/verify-compact-mechanics.py',ROOT/'scripts/pcb/board_b_layout.py',
                           ...
                           ROOT/'boards/board-p/mechanical.json',ROOT/'.claude/skills/component-project-passives/facts.json',
    ...
    report={'schema_version':1,'status':'PASS','pcb_sha256':source_hashes,'generator_sha256':generator_hash,
            ...
            'pd_transform':'P(x,y) -> B(y,x)','board_p_top_z_mm':contract['pd_stack']['nominal_board_p_top_z_mm'],
