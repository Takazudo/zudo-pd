...
def verify_pd_mating(fps):
    ...
    require(pd_to_board_b((6.55,37.7))==(37.7,6.55) and pd_to_board_b((19.25,37.7))==(37.7,19.25),
            'PD transform must be P(x,y) -> B(y,x); in-plane rotation reverses the net order')
    contract=json.loads((ROOT/'boards/board-b/mechanical.json').read_text())['pd_stack']
    require(contract['point_transform']=='P(x,y) -> B(y,x)' and contract['nominal_board_p_top_z_mm']==11.1,
...
def main():
    ...
    require(near(xy(fps['C5']),stack['c5_envelope']['center_mm']) and fps['C5'].GetLayer()==p.F_Cu
            and abs(fps['C5'].GetOrientationDegrees()%360-90)<.001,
