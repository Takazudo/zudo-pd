...
from board_b_layout import WIDTH, HEIGHT, PLACEMENTS, MOUNTING_HOLES, FIDUCIALS, route_offset, edge_primitives
...
def main():
    ...
    for x,y in [(40,54),(48,56),(58,60),(50,64)]:
        via=p.PCB_VIA(board);via.SetPosition(mm(x,y));via.SetWidth(p.FromMM(1.0));via.SetDrill(p.FromMM(.4))
    ...
    for text,x,y,size in [('zudo-pd B / guarded prototype',80,2.5,.8),
