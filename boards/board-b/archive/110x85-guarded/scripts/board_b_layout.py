...
PLACEMENTS = {
 ...
 'J6':(6,37,270,'bottom'),'J9':(6,49,270,'bottom'),'J7':(6,61,270,'bottom'),'J8':(6,73,270,'bottom'),
...
FIXED_REFS = BUCK_REFS | TOP_LDO_REFS | {'J5','P1','J6','J9','J7','J8','J10','J11','C5','C46','TP3','TP4','TP5'}
...
def edge_primitives():
    ...
    contract=json.loads((root/'3dp-files/faston-cover/pcb-contract.json').read_text())
    edges=[{'type':'line','start':[0,0],'end':[WIDTH,0]},
           {'type':'line','start':[WIDTH,0],'end':[WIDTH,HEIGHT]},
           {'type':'line','start':[WIDTH,HEIGHT],'end':[0,HEIGHT]}]
    # Left side is traced upward, through the reverse of each T-notch path.
    last=[0,HEIGHT]
    for notch in reversed(contract['notch_geometry']['notches']):
        path=notch['edge_path_top_to_bottom']
        edges.append({'type':'line','start':last,'end':path[-1]['end']})
        for primitive in reversed(path):
            rev={**primitive,'start':primitive['end'],'end':primitive['start']}
            edges.append(rev)
        last=path[0]['start']
    edges.append({'type':'line','start':last,'end':[0,0]})
    return edges
