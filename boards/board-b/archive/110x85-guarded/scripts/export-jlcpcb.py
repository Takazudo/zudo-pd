...
def check_cover_freshness():
    """Refuse printed parts verified against an older PCB or component model."""
    folder=ROOT/'3dp-files/faston-cover'
    report=json.loads((folder/'generated/verification.json').read_text())
    require(report.get('status')=='PASS','Full populated cover verification is required')
    require(report.get('verification_script_sha256')==digest(folder/'verify.py'),'Cover verification script changed')
    require(report.get('contract_checker_sha256')==digest(folder/'contract.py'),'Cover contract checker changed')
    if 'preview_finalizer_sha256' in report:
        require(report['preview_finalizer_sha256']==digest(folder/'finalize-preview.py'),'Cover preview finalizer changed')
    require(report.get('pcb_sha256')==digest(ROOT/'boards/board-b/board-b.kicad_pcb'),'Cover verification PCB is stale')
    require(report.get('contract_sha256')==digest(folder/'pcb-contract.json'),'Cover contract verification is stale')
    require(report.get('board_p',{}).get('pcb_sha256')==digest(ROOT/'boards/board-p/board-p.kicad_pcb'),'Cover stacked-P verification is stale')
    for name,sha in report.get('model_sources_sha256',{}).items():require(digest(ROOT/name)==sha,'Cover component model changed: '+name)
    require(report.get('model_sources_sha256'),'Cover report lacks populated model evidence')
    for name,sha in report.get('preview_sha256',{}).items():require(digest(folder/'generated/preview'/name)==sha,'Cover assembly preview changed: '+name)
    manifest=json.loads((folder/'generated/manifest.json').read_text())
    instance=manifest.get('pcb_instance_check')
    require(isinstance(instance,dict) and instance.get('status')=='PASS','Cover manifest lacks full PCB verification')
    require(instance.get('verification_sha256')==digest(folder/'generated/verification.json'),'Cover manifest verification report is stale')
    require(instance.get('board_b_sha256')==report['pcb_sha256'] and instance.get('board_p_sha256')==report['board_p']['pcb_sha256'],'Cover manifest board hashes differ')
    require(manifest.get('preview_files_sha256')==report.get('preview_sha256') and report.get('preview_sha256'),'Cover manifest preview hashes differ')
    for name,sha in manifest['source_sha256'].items():
        path=(ROOT/name) if name.startswith('footprints/') else (folder/name)
        require(digest(path)==sha,'Cover generator input changed: '+name)
    for part in manifest['parts']:require(digest(folder/part['file'])==part['sha256'],'Print STL changed: '+part['file'])
    for name,sha in manifest['assembly_files'].items():require(digest(folder/'generated'/name)==sha,'Cover assembly asset changed: '+name)
    print_files=[folder/part['file'] for part in manifest['parts']]
    print_files.extend(folder/name for name in ('pcb-contract.json','README.md','generated/manifest.json','generated/verification.json'))
    require(len({path.name for path in print_files})==len(print_files),'Duplicate print archive basename')
    with zipfile.ZipFile(folder/'generated/print-parts.zip') as archive:
        require(archive.testzip() is None,'Corrupt print ZIP')
        require(len(archive.namelist())==len(print_files) and set(archive.namelist())=={path.name for path in print_files},'Print ZIP contents mismatch')
        for path in print_files:require(archive.read(path.name)==path.read_bytes(),'Stale print ZIP member: '+path.name)
...
def export_board(name, output, cli, inventory, kicad_python):
    ...
    if name=='board-b':
        dependencies.update(path for path in (ROOT/'3dp-files/faston-cover').rglob('*') if path.is_file() and '__pycache__' not in path.parts and path.suffix in ('.py','.json','.md','.stl','.step','.wrl','.png','.zip','.kicad_pcb','.kicad_pro'))
...
def main():
    ...
    if 'board-b' in args.boards:check_cover_freshness()
    ...
    if 'board-b' in args.boards:
        shutil.copytree(ROOT/'3dp-files/faston-cover',output/'mechanical/faston-cover',ignore=shutil.ignore_patterns('__pycache__','*.pyc'))
