...
from board_b_layout import WIDTH, HEIGHT, PLACEMENTS, MOUNTING_HOLES, FIDUCIALS, route_offset, edge_primitives, GROUND_STITCHES, SUPPORT_REFS, POGO_LEGEND_CENTER
...
def main():
    ...
    for ref,comp in spec.COMPONENTS.items():
        ...
        if ref in ('P1','TP3','TP4','TP5'):
            fp.Reference().SetPosition(mm(pos[0],5.8 if ref=='P1' else 4.5))
    ...
    route('U2.1',[(41.8,14.05),(41.8,10.6),(40.5,10.6),(40.5,9.75)],.25,absolute=True)
    route('U2.1',[(41.75,11.75),(41.8,11.75)],.25,absolute=True)
    ...
    for i,(x,y) in enumerate(MOUNTING_HOLES,1):
        name='MountingHole_HC11_3mm' if 'H'+str(i) in SUPPORT_REFS else 'MountingHole_M3'
        fp=p.FootprintLoad(str(ROOT/'footprints/kicad/zudo-power.pretty'),name)
        fp.SetFPID(p.LIB_ID('zudo-pd',name));fp.SetReference('H'+str(i))
    ...
    for text,x,y,size in [('-12 / RED',50.29,68.5,0.8),('-12 / RED',90.39,68.5,0.8),
    ...
    for fp in board.GetFootprints():
        ...
        if fp.GetReference()=='R1':fp.Reference().SetPosition(mm(40.5,5.5))
        if fp.GetReference()=='J5':fp.Reference().SetPosition(mm(34.4,17.8))
        special={'C5':(18,12),'C46':(28,3.3),'C32':(26,20),'R3':(22.8,27.7),'R4':(23.6,26),'D2':(33.75,30.4),'C8':(30.7,41)}
        if fp.GetReference() in special:fp.Reference().SetPosition(mm(*special[fp.GetReference()]))
        if fp.GetReference() in ('C36','C37'):
            fp.Reference().SetPosition(mm(65,7.8 if fp.GetReference()=='C36' else 15))
