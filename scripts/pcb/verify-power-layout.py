...
def main_path(board, source, capacitor, fuse, minimum_width_mm=1.0):
    """Prove a minimum-width trace path without zones or thin sense traces.
...
"""
    ...
    for item in board.GetTracks():
        ...
        if isinstance(item, p.PCB_VIA):
            ...
            if p.ToMM(item.GetWidth()) < minimum_width_mm - EPS_MM:
                continue
            tracks.append(item)
    ...
    require(target in seen and capacitor_node in seen,
            f'No complete >={minimum_width_mm:g} mm output-to-capacitor/PTC power path')
    return {'minimum_trace_width_mm': minimum_width_mm, 'narrow_feedback_and_zones_excluded': True,
...
def verify(path):
    ...
    input_paths=[]
    for target,pin in [('J5','2'),('U3','1'),('U4','1'),('C39','1')]:
        result=main_path(board,pad('J5','1'),pad('C5','1'),pad(target,pin),1.5)
        input_paths.append({'source':'J5.1','input_bulk':'C5.1','destination':target+'.'+pin,**result})
    ...
    return {'board': 'board-b', 'pcb_sha256': hashlib.sha256(path.read_bytes()).hexdigest(),
            'terminal_load_paths':terminal_paths,'terminal_ground_both_planes':'PASS','pd_input_paths':input_paths,
...
def self_test(path):
    ...
    for track in board.GetTracks():
        if isinstance(track,p.PCB_TRACK) and not isinstance(track,p.PCB_VIA) and track.GetNetname()=='+15V INPUT':
            track.SetWidth(p.FromMM(1.0))
    rejects('narrowed PD input feed',
            lambda:main_path(board,pads['J5.1'],pads['C5.1'],pads['U3.1'],1.5),'No complete >=1.5 mm')
